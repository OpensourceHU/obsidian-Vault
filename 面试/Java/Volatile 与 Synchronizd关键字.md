...
但是CPU是速度极快的，丝毫的等待都是不可忍受的，因此又做了一系列优化，即将数据的同步改为异步， 将指令重排序：一个CPU在读写共享变量时首先发现其它CPU正在修改，则做一个标记，先去做别的指令  只要满足as if serial就行
...
1. 禁止编译器对其修饰变量进行指令重排
2. 保证其修饰的变量对所有线程可见
volatile关键字屏蔽了指令重排,使修饰的变量强制刷新回内存,并通过缓存一致性协议通知其它线程  其它线程每次读取volatile变量则从内存读取(总线嗅探)  这样就保证了volatile修饰的变量在每个线程中读到的都是最新的。
...
![[assets/image-20230312171629982.png]]
...
所谓自旋，其实就是让CPU忙等，如果争抢的资源可以很快被释放，那么自旋的开销就比较小, 小于让操作系统管程来接收处理互斥

如果线程2 尝试获取锁, 发现对象头里已经记录了当前对象是一个轻量级锁 则将当前对象记录在自己的锁空间Lock Record内 而对象本身也记录下这个Lock Record的指针
...
#### synchronized对象锁可重入吗
显然, 平时使用的情况来看 对象锁是可重入的
在反编译synchronized包裹的代码时, 发现: 进入竞态区域时  会有monitor Enter指令,而退出时会有monitor Exit指令
monitor中维护了一个计数器, 表示当前资源的加锁个数, 以此实现可重入
...
注意,CAS这里假设的是, 如果换入前比较之前读到的oldValue与当前内存中的值是否一致, 若一致,则认为没有发生冲突; 若不一致则认为在计算新值的过程中有其它程序对oldValue进行了修改 之前读到的oldValue是过期的了需要放弃操作 自选等待
这个判断逻辑引入了[[ABA问题]], 解决方法是加版本号
